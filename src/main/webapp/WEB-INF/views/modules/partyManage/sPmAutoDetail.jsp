...
<div id="container">
	<form:form id="inputForm" modelAttribute="sPmAuto" action="" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<h3>自传上传：</h3>
		 <div class="controls-group" id="viewImg">
            ...
            <div class="controls">
                ...
                <c:choose>
                    <c:when test="${sPmAuto.fileUrl == null }">
                        <a href="#" role="button" class="btn">下载</a></c:when>
                    <c:otherwise>
                        <a href="${ctx}/partyManage/public/downfile?fileUrl=${sPmAuto.fileUrl}" role="button"
                           class="btn">下载</a>
                    </c:otherwise>
                </c:choose>
            </div>
            ...
            <div class="controls">
                <input name="uploadTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
                       value="<fmt:formatDate value="${sPmAuto.uploadTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
            </div>
        </div>
